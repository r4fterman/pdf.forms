...
public class PdfDocumentLayout {
	private List document = new ArrayList();

	public void addPage(boolean isPdfPage) {
		document.add(new Page(isPdfPage));
	}

	public void addPage(boolean isPdfPage, String pdfPath, int pdfPageNumber) {
		document.add(new Page(isPdfPage, pdfPath, pdfPageNumber));
	}

	public Page getPage(int pageNumber) {
		return (Page) document.get(pageNumber - 1);
	}

	public List getPdfPages(){
...
//	public boolean containsPdfPage(){
//		for (Iterator it = document.iterator(); it.hasNext();) {
//			Page page = (Page) it.next();
//			if (page.isPdfPage) 
//				return true;
...
//	}
	...
	public static class Page {
		...
		Page(boolean isPdfPage) {
			this.isPdfPage = isPdfPage;
		}
		
		Page(boolean isPdfPage, String pdfPath, int pdfPageNumber) {
			this(isPdfPage);
