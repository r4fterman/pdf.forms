...
public class LayoutPropertiesPanel extends JPanel/*extends BasicPropertiesPanel*/ implements TristateCheckBoxParent {
    ...
    private void initComponents() {
        ...
        anchorLocationBox.addActionListener(new java.awt.event.ActionListener() {
        ...
        rotate0.setName("0");
        rotate0.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                updateRotation(evt);
            }
        });
        ...
        captionLocationBox.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                updateCaptionPosition(evt);
            }
        });
    ...
    private void updateCaptionPosition(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateCaptionPosition

        Set widgets = widgetsAndProperties.keySet();

        Object captionPosition = captionLocationBox.getSelectedItem();
        ...
        for (Iterator it = widgets.iterator(); it.hasNext();) {
            ...
            if (widget.isComponentSplit() && captionPosition != null) {
                ...
                Element captionPositionElement = XMLUtils.getPropertyElement(widgetProperties, "Position");
    ...
    }//GEN-LAST:event_updateCaptionPosition

    private void updateRotation(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateRotation
        ...
        for (Iterator it = widgets.iterator(); it.hasNext();) {
            ...
            Element rotationElement = XMLUtils.getPropertyElement(widgetProperties, "Rotation");

            rotationElement.getAttributeNode("value").setValue(alignment);
        }

    }//GEN-LAST:event_updateRotation

    private void updateAnchor(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_updateAnchor

        Set widgets = widgetsAndProperties.keySet();

        Object anchor = anchorLocationBox.getSelectedItem();
        ...
        for (Iterator it = widgets.iterator(); it.hasNext();) {
            ...
            if (anchor != null) {
                ...
                Element anchorElement = XMLUtils.getPropertyElement(widgetProperties, "Anchor");

                anchorElement.getAttributeNode("value").setValue(anchor.toString());
            }
        }

    }//GEN-LAST:event_updateAnchor

    private void updateSizeAndPosition(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_updateSizeAndPosition

        Integer[] props = new Integer[4];

        if (!xBox.getText().equals("mixed")) {
            String xText = xBox.getText().replaceAll("cm", "");
            ...
            props[0] = new Integer((int) (Math.round(x) + IMainFrame.INSET));
        }
        if (!yBox.getText().equals("mixed")) {
            String yText = yBox.getText().replaceAll("cm", "");
            ...
            props[1] = new Integer((int) (Math.round(y) + IMainFrame.INSET));
        }
        if (!widthBox.getText().equals("mixed")) {
            ...
            props[2] = new Integer((int) Math.round(width));
        }
        if (!heightBox.getText().equals("mixed")) {
            ...
            props[3] = new Integer((int) Math.round(height));
        }
        ...
        for (Iterator it = widgets.iterator(); it.hasNext();) {
            ...
            widget.setLayoutProperties((Element) widgetsAndProperties.get(widget));
        }

        designerPanel.repaint();

    }//GEN-LAST:event_updateSizeAndPosition

    public void checkboxClicked(MouseEvent e) {

        Set widgets = widgetsAndProperties.keySet();

        TristateCheckBox.State xExpandState = (((TristateCheckBox) xExpandToFitBox).getState());
        TristateCheckBox.State yExpandState = (((TristateCheckBox) yExpandToFitBox).getState());

        for (Iterator it = widgets.iterator(); it.hasNext();) {
            IWidget widget = (IWidget) it.next();

            List layoutProperties = XMLUtils.getElementsFromNodeList(
                    ((Element) widgetsAndProperties.get(widget)).getChildNodes());

            /** add size & position properties */
            List sizeAndPosition = XMLUtils.getElementsFromNodeList(
                    ((Element) layoutProperties.get(0)).getChildNodes());

            if (xExpandState != TristateCheckBox.DONT_CARE) {
                Element xExpand = (Element) sizeAndPosition.get(4);
            ...
            if (yExpandState != TristateCheckBox.DONT_CARE) {
                Element yExpand = (Element) sizeAndPosition.get(5);
...
//        TristateCheckBox.State xExpandStateToUse = null, yExpandStateToUse = null;
        ...
        for (Iterator it = widgetsAndProperties.keySet().iterator(); it.hasNext();) {
            ...
            Element props = (Element) widgetsAndProperties.get(widget);
            
            /** add size & position properties */
            Element sizeAndPosition = (Element) props.getElementsByTagName("sizeandposition").item(0);

            String xCord = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "X");
            String width = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "Width");
            String yCord = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "Y");
            String height = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "Height");
...
//            boolean yExpand = Boolean.valueOf(XMLUtils.getAttribute(sizeAndPosition, 5)).booleanValue();
            ...
            String anchor = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "Anchor");
            String rotation = XMLUtils.getAttributeFromChildElement(sizeAndPosition, "Rotation");

            /** add caption properties */
            String captionPosition = null;
            if (widget.isComponentSplit()) {
                Element caption = (Element) props.getElementsByTagName("caption").item(0);
...
//        ((TristateCheckBox) yExpandToFitBox).setState(yExpandStateToUse);
        ...
        anchorLocationBox.setSelectedItem(anchorLocationToUse.equals("mixed") ? null : anchorLocationToUse);

        if (rotationToUse.equals("mixed")) {
            buttonGroup1.setSelected(new JToggleButton("").getModel(), true);
        } else {

            /** use reflection to set the required rotation button selected */
            try {
                Field field = getClass().getDeclaredField("rotate" + rotationToUse);
        ...
        if (isComponentSplit) {
            captionLocationBox.setSelectedItem(captionPositionToUse.equals("mixed") ? null : captionPositionToUse);
        } else {
...
//        reserveBox.setText(reserve);
    ...
    private javax.swing.JComboBox anchorLocationBox;
    private javax.swing.JTextField bottomMarginBox;
    private javax.swing.ButtonGroup buttonGroup1;
    private javax.swing.JComboBox captionLocationBox;
